...
CPU_state cpu = {};
uint64_t g_nr_guest_inst = 0;  //记录客户指令的计数器,
...
// s, 这个结构体用于存放在执行一条指令过程中所需的信息, 包括指令的PC, 下一条指令的PC等. 还有一些信息是ISA相关的,
//盖了指令周期的所有阶段: 取指, 译码, 执行, 更新PC
...
//者模拟了CPU的工作方式: 不断执行指令. 具体地, 代码将在一个for循环中不断调用exec_once()函数
//
