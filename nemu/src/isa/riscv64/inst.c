...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  ...
  switch (type) {
    ...
    case TYPE_J: src1I(immJ(i)) ;  printf("src1 is %ld\n",(*src1)); break;  //mm[20|10:1|11|19:12]  rd  opcode 
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  //INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    ,J, R(dest)=s->pc +4,  s->pc=s->pc+src1);  ///有条件跳转    src2 是立即数 jal  ！！！！
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    ,J, s->pc=s->pc+src1); 
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   ,I, R(dest)=s->pc +4;s->pc=((src2+src1)&(-1)) );       ///有条件跳转    src2 是立即数 jal  ！！！！     
